...
pub extern "C" fn pedersen_commit(
    ...
    key_bytes: *const u8,
    values: *const u8,
    commitment: &mut [u8; COMMITMENT_LENGTH],
    randomness: &mut [u8; RANDOMNESS_LENGTH],
) -> i32 {
    assert!(!key_bytes.is_null(), "Null pointer");
    assert!(!values.is_null(), "Null pointer");
    let key_slice: &[u8] =
        unsafe { slice::from_raw_parts(key_bytes, (n + 1) * GROUP_ELEMENT_LENGTH) };
...
#[no_mangle]
#[allow(clippy::not_unsafe_ptr_arg_deref)]
pub extern "C" fn pedersen_random_values(n: usize, values_bytes: *mut u8) {
    let mut csprng = thread_rng();
    let vs = Value::generate(n, &mut csprng);
    let v_slice: &mut [u8] =
        unsafe { slice::from_raw_parts_mut(values_bytes, n * FIELD_ELEMENT_LENGTH) };
