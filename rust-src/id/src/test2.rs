...
use elgamal::{
    cipher::Cipher, message::Message as ElgamalMessage, public::PublicKey as EncryptionKey,
    secret::SecretKey as DecryptionKey,
};
use pairing::Field;
use pedersen_scheme::{
...
pub fn test() {
    ...
    let (v, cmm_prf_sharing_coeff, _) =
        compute_prf_key_data(&prf_key, &(ar_parameters.clone(), 5), &ck);
    for i in 0..8 {
        let message = ar_dec_keys[i].decrypt(&v[i].encrypted_share);
        assert_eq!(
            <G1 as Curve>::one_point().mul_by_scalar(&v[i].share),
            message.0
        );
    }
    ...
    assert_eq!(
        prf_key.0,
        reveal::<G1>(good_shares.iter().map(|x| x).collect())
    );
    ...
    assert_ne!(
        prf_key.0,
        reveal::<G1>(bad_shares.iter().map(|x| x).collect())
    );

    for item in v.iter() {
        assert!(ck.open(
            &Value(item.share),
            &item.randomness_cmm_to_share,
            &item.cmm_to_share
        ));
    }
    ...
    for i in 0..v.len() {
        ...
        assert!(com_enc_eq::verify_com_enc_eq(
            &[],
            &(<G1 as Curve>::one_point(), public_key, ck.0, ck.1),
            &(e_1, e_2, e_3),
            &proof
        ));
    }
...
// }
// #[inline]
// fn compute_prf_key_data<C:Curve> (prf_key: &prf::SecretKey<C>, ar_parameters:
// &(Vec<ArInfo<C>>, u64), commitment_key: &PedersenKey<C>)->
// (Vec<SingleArData<C>>, Vec<(u64, Commitment<C>)>){ let n = ar_parameters.0.
// len() as u64; let t = ar_parameters.1;
// let mut csprng = thread_rng();
// let prf_key_scalar = prf_key.0;
// let (cmm_prf, cmm_prf_rand) = commitment_key.commit(&Value(prf_key_scalar),
// &mut csprng); let sharing_data = share::<C, ThreadRng>(&prf_key_scalar, n, t,
// &mut csprng); let mut cmm_sharing_coefficients:Vec<(u64, Commitment<C>)> =
// Vec::with_capacity(t as usize); cmm_sharing_coefficients.push((0, cmm_prf));
// let mut cmm_coeff_randomness: Vec<(u64, Randomness<C>)> =
// Vec::with_capacity(t as usize); cmm_coeff_randomness.push((0, cmm_prf_rand));
// for i in 1..(t as usize){
// let (cmm, rnd) = commitment_key.commit(&Value(sharing_data.coefficients[i as
// usize-1].1), &mut csprng); cmm_sharing_coefficients.push((i as u64, cmm));
...
// let (cmm, rnd) = commitment_to_share(i as u64, &cmm_sharing_coefficients,
// &cmm_coeff_randomness); let proof = com_enc_eq::prove_com_enc_eq(&mut csprng,
// &[], &(cipher.0, cipher.1, cmm.0), &(rnd2, share, rnd.0), &(C::one_point(),
// pk.0, commitment_key.0, commitment_key.1)); let ar_data =
// SingleArData{
...
//
// #[inline(always)]
// fn commitment_to_share<C:Curve>(share_number: u64, coeff_commitments:
// &Vec<(u64,Commitment<C>)>, coeff_randomness: &Vec<(u64,Randomness<C>)>)
...
// for i in 1..(deg+1) {
// let j_pow_i: C::Scalar = C::scalar_from_u64(share_number).unwrap().pow([i as
// u64]); let (s, Commitment(cmm_point)) = coeff_commitments[i];
