...
pub fn verify_attribute<C: Curve, AttributeType: Attribute<C::Scalar>>(keys: &PedersenKey<C>, attribute: &AttributeType, r: &PedersenRandomness<C>, c: &Commitment<C>) -> bool {
    let s = Value::new(attribute.to_field_element());
    keys.open(&s, &r, &c)
}

pub fn verify_attribute_range<C: Curve, AttributeType: Attribute<C::Scalar>>(keys: &PedersenKey<C>, gens: &Generators<C>, lower: &AttributeType, upper: &AttributeType, c: &Commitment<C>, proof: &RangeProof<C>) -> bool {
    ...
    verify_efficient(&mut transcript, 64, &[com_delta_minus_b_plus_2n, com_delta_minus_a], proof, gens, keys).is_ok()
}

/// Function for verifying account ownership. The arguments are 
/// - public_data - the public keys (and threshold) of the prover. These should be read from chain by looking up the account. 
///   If they are not present on chain, the verifier should reject the proof.
...
/// 
/// The function outputs a bool, indicating whether the proof is correct or not, i.e., 
/// it checks the signatures inside the proof. 
pub fn verify_account_ownership(public_data: &CredentialPublicKeys,  account: AccountAddress, challenge: &[u8], proof: &AccountOwnershipProof) -> bool {
...
mod tests {
    ...
    fn test_verify_account_ownership() {
        ...
        assert!(verify_account_ownership(&pub_data, account_address, challenge, &proof));
    }
}