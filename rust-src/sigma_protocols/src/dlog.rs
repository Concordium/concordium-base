...
#[derive(Clone)]
pub struct DlogProof<T:Curve>{challenge: T::Scalar, randomised_point: T, witness: T::Scalar}

pub fn prove_dlog<T:Curve, R:Rng>(csprng: &mut R, public: &T, secret: &T::Scalar, base: &T) -> DlogProof<T>{
    ...
    while !suc{
        ...
        match T::bytes_to_scalar(&hash){
            Err(_) => {},
            Ok(x) =>{
                if x==T::Scalar::zero() { println!("x = 0");}
                else{
                    challenge = x;
    ...
    DlogProof{challenge, randomised_point, witness} 
}
...
pub fn verify_dlog<T:Curve>(base: &T, public: &T,  proof: &DlogProof<T>) -> bool{
    ...
    match T::bytes_to_scalar(&hash){
        Err(_) => false,
        Ok(c) => 
            proof.randomised_point == public.mul_by_scalar(&proof.challenge).plus_point(&base.mul_by_scalar(&proof.witness))
        &&
        c == proof.challenge
    }
}
