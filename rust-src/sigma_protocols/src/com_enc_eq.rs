...
#[derive(Clone)]
pub struct ComEncEqProof<T:Curve>{challenge: T::Scalar, randomised_points: (T, T, T), witness: (T::Scalar, T::Scalar, T::Scalar)}

pub fn prove_com_enc_eq<T:Curve, R:Rng>(csprng: &mut R, public: &(T, T, T), secret: &(T::Scalar, T::Scalar, T::Scalar), base: &(T, T, T, T)) -> ComEncEqProof<T>{
    ...
    ComEncEqProof{challenge, randomised_points, witness} 
}

pub fn verify_com_enc_eq<T:Curve>(base: &(T, T, T, T), public: &(T,T, T),  proof: &ComEncEqProof<T>) -> bool{
    let mut hasher = Sha256::new();
    let (g_1, h_1, g, h) = base;
    ...
    match T::bytes_to_scalar(&hash){
        Err(_) => false,
        Ok(c) =>{
            if c != proof.challenge {return false};
            let b_1 = a_1 == g_1.mul_by_scalar(&w_1).plus_point(&e_1.mul_by_scalar(&c));
            let b_2 = a_2 == g_1.mul_by_scalar(&w_2).plus_point(&h_1.mul_by_scalar(&w_1)).plus_point(&e_2.mul_by_scalar(&c));
            let b_3 = a_3 == g.mul_by_scalar(&w_2).plus_point(&h.mul_by_scalar(&w_3)).plus_point(&e_3.mul_by_scalar(&c));
            b_1 && b_2 && b_3
        }
    }
}
...
pub fn test_com_enc_eq(){
    ...
    for i in 0..100{
        ...
        let (g_1, h_1, g, h) = (G1Affine::generate(&mut csprng), G1Affine::generate(&mut csprng), G1Affine::generate(&mut csprng), G1Affine::generate(&mut csprng));
        ...
        let proof = prove_com_enc_eq::<G1Affine,ThreadRng >(&mut csprng, &public, &(s_1, s_2, s_3), &(g_1, h_1, g,h));
