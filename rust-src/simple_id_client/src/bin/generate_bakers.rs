...
fn main() {
    let app = App::new("Generate bakers with accounts for inclusion in genesis.")
        ...
        .arg(
            Arg::with_name("ip-data")
                ...
                .value_name("FILE")
                .help(
                    "File with all information about the identity provider that is going to sign \
                     all the credentials.",
                )
                .required(true),
        )
    ...
    for baker in 0..num_bakers {
        ...
        let account_data_json = json!({
            "schemeId": "Ed25519",
            "address": AccountAddress::new(SchemeId::Ed25519, &acc_data.verify_key).to_json(),
        ...
        let public_baker_data = json!({
            ...
            "account": json!({
                "schemeId": "Ed25519",
                "address": AccountAddress::new(SchemeId::Ed25519, &acc_data.verify_key).to_json(),
                "verifyKey": json_base16_encode(&account_kp.public.to_bytes()),
