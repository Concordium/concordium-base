...
pub fn share<C: Curve, R: Rng>(
    ...
    let coefficients: Vec<(u64, C::Scalar)> =
        (1..=deg).map(|x| (x, C::generate_scalar(csprng))).collect();
...
fn lagrange<C: Curve>(kxs: &[u64], i: u64) -> C::Scalar {
...
pub fn reveal<C: Curve>(shares: &[(u64, C::Scalar)]) -> C::Scalar {
...
pub fn reveal_in_group<C: Curve>(shares: &[(u64, C)]) -> C {
    let kxs: Vec<u64> = shares.iter().map(|(fst, _)| *fst).collect();
