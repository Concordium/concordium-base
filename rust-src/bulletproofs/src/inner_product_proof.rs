...
#[allow(non_snake_case)]
#[derive(Clone, Serialize)]
pub struct InnerProductProofBetter<C: Curve> {
    pub lr_vec: Vec<(C, C)>,
    pub a:     C::Scalar,
    pub b:     C::Scalar,
}
...
#[allow(dead_code)]
// #[cfg(test)]
pub fn prove_inner_product_without_vecs<C: Curve>(
    transcript: &mut Transcript,
    G_slice: &[C],
    H_slice: &[C],
    Q: &C,
    ...
    let mut L = Vec::with_capacity(k);
    let mut R = Vec::with_capacity(k);
    let mut a_slice = a_slice;
    let mut b_slice = b_slice;
    let mut G_slice = G_slice;
    let mut H_slice = H_slice;
    let mut a : Vec<_>;
    let mut b : Vec<_>;
    let mut G : Vec<_>;
    let mut H : Vec<_>;
    for _j in 0..k {
        n = G_slice.len();
        let a_lo = &a_slice[..n / 2];
        let a_hi = &a_slice[n / 2..];
        let G_lo = &G_slice[..n / 2];
        let G_hi = &G_slice[n / 2..];
        let b_lo = &b_slice[..n / 2];
        let b_hi = &b_slice[n / 2..];
        let H_lo = &H_slice[..n / 2];
        let H_hi = &H_slice[n / 2..];
        let a_lo_G_hi = multiexp(G_hi, a_lo);
        let a_hi_G_lo = multiexp(G_lo, a_hi);
        let b_hi_H_lo = multiexp(H_lo, b_hi);
        let b_lo_H_hi = multiexp(H_hi, b_lo);
        ...
        let mut a_tmp = Vec::with_capacity(a_lo.len());
        let mut b_tmp = Vec::with_capacity(a_lo.len());
        let mut G_tmp = Vec::with_capacity(a_lo.len());
        let mut H_tmp = Vec::with_capacity(a_lo.len());
        let G_scalars = [u_j_inv, u_j]; // For faster way
        let H_scalars = [u_j, u_j_inv]; // For faster way
        for i in 0..a_lo.len() {
            // Calculating new a vector:
...
#[allow(dead_code)]
// #[cfg(test)]
pub fn prove_inner_product_better<C: Curve>(
    transcript: &mut Transcript,
    G_slice: &[C],
    H_slice: &[C],
    Q: &C,
...
/// Precondictions:
/// G_slice, H_slice, a_slice and b_slice should all be of the same length, and this
/// length must a power of 2.
#[allow(non_snake_case)]
#[allow(dead_code)]
pub fn prove_inner_product_with_scalars_better<C: Curve>(
    transcript: &mut Transcript,
    G_slice: &[C],
    H_slice: &[C],
    H_prime_scalars: &[C::Scalar],
...
/// Both u_sq and u_inv_sq have to be of equal length k, and s has to be of length 2^k. 
...
#[allow(non_snake_case)]
#[allow(clippy::many_single_char_names)]
pub fn verify_scalars_better<C: Curve>(
    transcript: &mut Transcript,
    n: usize,
    proof: &InnerProductProofBetter<C>,
) -> VerificationScalars<C> {
    // let n = G_vec.len();
    ...
    let mut s_0 = C::Scalar::one();
    for j in 0..k {
        transcript.append_point(b"Lj", &L_R[j].0);
        transcript.append_point(b"Rj", &L_R[j].1);
        let u_j: C::Scalar = transcript.challenge_scalar::<C>(b"uj");
        let u_j_inv = u_j.inverse().unwrap(); // TODO be careful here
        s_0.mul_assign(&u_j_inv);
        let mut u_j_sq = u_j;
...
mod tests {
    ...
    #[test]
    #[allow(non_snake_case)]
    fn test_inner_product_proof_without_vecs() {
        let rng = &mut thread_rng();
        let n = 32 * 16;
        let mut G_vec = vec![];
        let mut H_vec = vec![];
        ...
        // Producing inner product proof with vector length = n
        let proof = prove_inner_product_better(
            &mut transcript,
            &G_vec,
            &H_vec,
            &Q,
