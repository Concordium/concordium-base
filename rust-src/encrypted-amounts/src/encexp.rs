...
use curve_arithmetic::{Curve, Value, multiexp};
use elgamal::{
    cipher::{Cipher, Randomness as ElgamalRandomness},
    public::PublicKey,
};
use id::sigma_protocols::{com_eq::Witness as ComEqWitness, com_eq::*, common::*, dlog::Witness as DlogWitness,  dlog::*, aggregate_dlog::*};
...
impl<C: Curve> SigmaProtocol for DlogEqual<C>{
    ...
    fn generate_witness(
        ...
        let w1 = self
            .dlog1
            .generate_witness(secret, state, &challenge)?; // The witnesses are the same, so does not matter which one
        Some(w1)
    }
...
mod test {
    ...
    fn test_gen_eq() {
        ...
        let dlog1 = Dlog{public: g1x, coeff: g1};
        let dlog2 = Dlog{public: g2x, coeff: g2};
    ...
    fn test_dlog_agg_eq() {
        ...
        let dlog = Dlog{public: gx, coeff: g};
        let g1xh1x1f1y1 = multiexp(&[g1, h1, f1], &[x, x1, y1]);
        let g2xh2x2f2y2 = multiexp(&[g2, h2, f2], &[x, x2, y2]);
        let agg1 = AggregateDlog{public: g1xh1x1f1y1, coeff: vec![g1, h1, f1]};
        let agg2 = AggregateDlog{public: g2xh2x2f2y2, coeff: vec![g2, h2, f2]};
        let protocol = DlogAndAggregateDlogsEqual{dlog, aggregate_dlogs: vec![agg1, agg2]};
        let secret = (Rc::new(x), vec![vec![Rc::new(x1), Rc::new(y1)], vec![Rc::new(x2), Rc::new(y2)]]);
        let challenge_prefix = generate_challenge_prefix(&mut csprng);
        let ro = RandomOracle::domain(&challenge_prefix);
