...
use concordium_contracts_common::{to_bytes, Amount};
...
//         runner:        Runner,
//     },
//     #[structopt(name = "update", about = "Invoke a receive method of a
// module.")]     Receive {
//         #[structopt(
//             name = "contract",
//             long = "contract",
...
pub fn main() -> anyhow::Result<()> {
    ...
    match cmd {
        // TODO: Make the run command work for both V0 and V1 contracts.
        //
        // Command::Run(_run_cmd) => {
        ...
        // ensure!(
        //     !(runner.parameter_bin_path.is_some() &&
        // runner.parameter_json_path.is_some()),
        //     "Both --parameter-bin and --parameter-json are supplied. Only
        // one parameter is \      allowed."
        // );

        // let module = fs::read(&runner.module).context("Could not read
        // module file.")?;

        // let module_schema_opt = if let Some(schema_path) =
        // &runner.schema_path {     let bytes =
        // fs::read(schema_path).context("Could not read schema file.")?;
        //     let schema = from_bytes(&bytes)
        //         .map_err(|_| anyhow::anyhow!("Could not deserialize
        // schema file."))?;     Some(schema)
        // } else {
        //     let res = utils::get_embedded_schema(&module);
        //     if let Err(err) = &res {
        //         eprintln!(
        //             "{}",
        //             warning_style.paint(format!("Could not use embedded
        // schema: {}", err))         );
        //     }
        //     res.ok()
        // };
        ...
        // let print_result = |state: v0::State, logs: v0::Logs| ->
        // anyhow::Result<()> {     for (i, item) in
        // logs.iterate().enumerate() {         eprintln!("{}:
        // {:?}", i, item)     }
        //     let state = &state.state;
        //     match (runner.ignore_state_schema,
        // &contract_schema_state_opt) {         (false,
        // Some(state_schema)) => {             let s =
        // state_schema
        // .to_json_string_pretty(&state)
        // .map_err(|_| anyhow::anyhow!("Could not encode state to
        // JSON."))?;             if
        // runner.schema_path.is_some() {
        // eprintln!("The new state is: (Using provided schema)\n{}", s)
        //             } else {
        //                 eprintln!("The new state is: (Using embedded
        // schema)\n{}", s)             }
        //         }
        //         _ => eprintln!(
        //             "The new state is: (No schema found for contract
        // state) {:?}\n",             state
        //         ),
        //     };

        //     if let Some(file_path) = &runner.out_bin {
        //         fs::write(file_path, &state).context("Could not write
        // state to file")?;     }
        //     if let Some(file_path) = &runner.out_json {
        //         contract_schema_opt.context(
        //             "Schema is required for outputting state in JSON. No
        // schema found for \              this contract.",
        //         )?;
        //         let schema_state =
        // contract_schema_state_opt.as_ref().context(
        //             "Schema is required for outputting state in JSON. No
        // schema found the \              state in this
        // contract.",         )?;
        //         let json_string = schema_state
        //             .to_json_string_pretty(&state)
        //             .map_err(|_| anyhow::anyhow!("Could not output
        // contract state in JSON."))?;         fs::
        // write(file_path, json_string).context("Could not write out the
        // state.")?;     }
        //     Ok(())
        // };

        // let parameter = if let Some(param_file) =
        // &runner.parameter_bin_path {     fs::read(&
        // param_file).context("Could not read parameter-bin file.")
        // } else if let Some(param_file) = &runner.parameter_json_path {
        //     if contract_schema_opt.is_none() {
        //         Err(anyhow::anyhow!(
        //             "No schema found for contract, a schema is required
        // for using \              --parameter-json."
        //         ))
        //     } else {
        //         let parameter_schema = contract_schema_func_opt
        //             .context("Contract schema did not contain a schema
        // for this parameter.")?;

        //         let file = fs::read(&param_file).context("Could not read
        // parameter file.")?;         let parameter_json:
        // serde_json::Value = serde_json::from_slice(&file)
        //             .context("Could not parse the JSON in parameter-json
        // file.")?;         let mut parameter_bytes =
        // Vec::new();
        ...
        //             &mut parameter_bytes,
        //         )
        //         .context("Could not generate parameter bytes using schema
        // and JSON.")?;         Ok(parameter_bytes)
        //     }
        // } else {
        //     Ok(Vec::new())
        // }
        ...
        //                     .context("Could not read init context
        // file.")?;
        ...
        //                 remaining_energy,
        //             } => {
        //                 eprintln!("Init call succeeded. The following
        // logs were produced:");
        // print_result(state, logs)?;
        // eprintln!(                     "Interpreter energy
        // spent is {}",                     runner.energy -
        // remaining_energy                 )
        //             }
        ...
        //                 reason,
        //             } => {
        //                 eprintln!("Init call rejected with reason {}.",
        // reason);                 eprintln!(
        ...
        //                 eprintln!("Init call terminated with out of
        // energy.")             }
        //         };
        //     }
        //     RunCommand::Receive {
        ...
        //             Some(context_file) => {
        //                 let ctx_content = fs::read(context_file)
        //                     .context("Could not read receive context
        // file.")?;
        // serde_json::from_slice(&ctx_content)
        ...
        //
        // Some(concordium_contracts_common::Amount::
        // from_micro_ccd(balance));         }

        //         // initial state of the smart contract, read from either
        // a binary or json file.         let init_state = match
        // (state_bin_path, state_json_path) {
        // (None, None) => bail!(                 "The current
        // state is required for simulating an update to a contract \
        //                  instance. Use either --state-bin or
        // --state-json."             ),
        //             (Some(_), Some(_)) => bail!(
        //                 "Only one state is allowed, choose either
        // --state-bin or --state-json."             ),
        //             (Some(file_path), None) => {
        //                 let mut file =
        //                     File::open(&file_path).context("Could not
        // read state file.")?;                 let metadata =
        //                     file.metadata().context("Could not read file
        // metadata.")?;                 let mut init_state =
        ...
        //                     .as_ref()
        //                     .context("A schema for the state must be
        // present to use JSON.")?;                 let file =
        //                     fs::read(&file_path).context("Could not read
        // state file.")?;                 let state_json:
        // serde_json::Value = serde_json::from_slice(&file)
        ...
        //                     &mut state_bytes,
        //                 )
        //                 .context("Could not generate state bytes using
        // schema and JSON.")?;                 state_bytes
        //             }
        //         };
        ...
        //             } => {
        //                 eprintln!(
        //                     "Receive method succeeded. The following logs
        // were produced."                 );
        //                 print_result(state, logs)?;
        //                 eprintln!("The following actions were
        // produced.");                 for (i, action) in
        // actions.iter().enumerate() {
        // match action {
        // v0::Action::Send {                             data,
        //                         } => {
        //                             let name_str =
        // std::str::from_utf8(&data.name)
        // .context("Target name is not a valid UTF8 sequence.")?;
        //                             eprintln!(
        //                                 "{}: send a message to contract
        // at ({}, {}), calling \
        ...
        //                             l,
        //                             r,
        //                         } => eprintln!("{}: AND composition of {}
        // and {}", i, l, r),
        // v0::Action::Or {                             l,
        //                             r,
        //                         } => eprintln!("{}: OR composition of {}
        // and {}", i, l, r),
        ...
        //                 reason,
        //             } => {
        //                 eprintln!("Receive call rejected with reason {}",
        // reason);                 eprintln!(
        //                     "Interpreter energy spent is {}",
        //                     runner.energy - remaining_energy
        //                 )
        ...
        //                 eprintln!("Receive call terminated with: out of
        // energy.")             }
        //         }
        //     }
        // }
...
fn print_contract_schema(
    ...
    // TODO: Print state schema for V0 contracts.
