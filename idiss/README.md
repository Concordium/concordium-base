...
Two identity issuance flows are supported. The version 1 flow is the current flow and should be used for new integrations. The version 0 flow is deprecated and is kept only for compatibility with the existing C# and Node.js exports. The version 0 flow involves the creation of an initial account, the version 1 flow does not.
...
The version 0 identity issuance flow is deprecated and consists of the following steps. The steps done by this library are marked with (*).
...
The exposed API consists of five functions `validate_request`, `create_identity_object`, `validate_request_v1`, `create_identity_object_v1`, and `validate_recovery_request`.
...
- the address of the initial account
...
To build the Go exports, do instead
  ```
...
of success, or an `Error` if the request is malformed.
...
timestamp is within 60 seconds of the present**. This is not checked by the
...
The library exposes five functions `validate_request_v1_cs`, `create_identity_object_v1_cs`, `validate_recovery_request_cs`, `validate_request_cs`, and `create_identity_object_cs` that can be imported in C# by
...
The Go wrapper in `../idiss-go` exposes three functions `ValidateRequestV1`,
`CreateIdentityObjectV1`, and `ValidateRecoveryRequest`.

```go
func ValidateRequestV1(
    global idiss.Versioned[idiss.GlobalContext],
    ipInfo idiss.Versioned[idiss.IPInfo],
    arsInfos idiss.Versioned[map[string]idiss.ARInfo],
    request idiss.IDObjectRequestV1,
) error
```

which validates the given request and returns `nil` if the request is valid, or
an error otherwise.
...
func CreateIdentityObjectV1(
    ...
    request idiss.IDObjectRequestV1,
    ipPrivateKey string,
) (idiss.IdentityCreationV1, error)
```

which returns either the version 1 identity object to be sent back to the user,
as well as the anonymity revocation record, or an error.

```go
func ValidateRecoveryRequest(
    global idiss.Versioned[idiss.GlobalContext],
    ipInfo idiss.Versioned[idiss.IPInfo],
    request idiss.IDRecoveryWrapper,
    now time.Time,
) error
```

which validates the given recovery request and returns `nil` if the request is
valid, or an error otherwise.

The recovery request has a property `timestamp` which is a unix timestamp in
seconds. The caller of `ValidateRecoveryRequest` must supply the current time,
and the Go wrapper checks that the timestamp is within 60 seconds of the
present before calling into Rust.
