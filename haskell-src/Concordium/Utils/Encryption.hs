...
decodeMaybeEncrypted getPwd json = do
  case AE.eitherDecodeStrict json of
    Left _ -> return $ (AE.eitherDecodeStrict json `embedErr` (\err -> "Error decoding JSON: " ++ err), False)
    Right encryptedJSON -> do
      pwd <- getPwd
      return $ (decryptJSON encryptedJSON pwd `embedErr` (("Error decoding encrypted JSON: " ++) . displayException), True)
