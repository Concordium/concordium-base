...
dummyRegId :: GlobalContext -> AccountAddress -> ID.CredentialRegistrationID
dummyRegId gc addr = ID.RegIdCred (generateGroupElementFromSeed gc (fromIntegral (IntHash.hash addr)))
...
dummyCredential :: GlobalContext -> AccountAddress -> SigScheme.VerifyKey -> ID.CredentialValidTo -> ID.CredentialCreatedAt -> ID.AccountCredential
dummyCredential gc addr key pValidTo pCreatedAt = ID.NormalAC (ID.CredentialDeploymentValues
    {
