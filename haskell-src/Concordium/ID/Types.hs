...
addressFromBytes bs =
    case base58CheckDecode' bs of
        ...
        Just x
            | BS.length x == accountAddressSize + 1 ->
                let version = BS.head x
                in  if version == 1
                        then return (AccountAddress (FBS.fromByteString (BS.tail x)))
...
addressFromRegId (RegIdCred fbs) = AccountAddress (FBS.FixedByteString addr) -- NB: This only works because the sizes are the same
  where
    SHA256.Hash (FBS.FixedByteString addr) = SHA256.hash (encode fbs)
...
getAccountInformation threshold credentials =
    AccountInformation
...
instance ToJSON CredentialAccount where
    toJSON (NewAccount keys threshold) =
        object
            [ "keys" .= keys,
              "threshold" .= threshold
            ]
...
instance ToJSON InitialCredentialAccount where
    toJSON (InitialCredentialAccount keys threshold) =
        object
            [ "keys" .= keys,
              "threshold" .= threshold
            ]
...
instance ToJSON CredentialDeploymentCommitments where
    toJSON CredentialDeploymentCommitments{..} =
        object
            [ "cmmPrf" .= cmmPrf,
