{-# LANGUAGE DataKinds #-}
{-# LANGUAGE DerivingStrategies #-}
{-# LANGUAGE GADTs #-}
{-# LANGUAGE OverloadedStrings #-}

-- | This module defines the genesis data format for the 'P11' protocol version.
module Concordium.Genesis.Data.P11 where

import Data.Serialize
import Data.Word

import Concordium.Common.Version
import qualified Concordium.Crypto.SHA256 as Hash
import qualified Concordium.Genesis.Data.Base as Base
import qualified Concordium.Genesis.Data.BaseV1 as BaseV1
import Concordium.Types

-- | Parameters used to migrate state from 'P0' to 'P11'.
data StateMigrationData = StateMigrationData
    deriving (Eq, Show)

instance Serialize StateMigrationData where
    put StateMigrationData = return ()
    get = return StateMigrationData

-- | Initial genesis data for the P11 protocol version.
data GenesisDataP11 = GDP11Initial
    { -- | The immutable genesis parameters.
      genesisCore :: !BaseV1.CoreGenesisParametersV1,
      -- | Serialized initial block state.
      genesisInitialState :: !(Base.GenesisState 'P11)
    }
    deriving (Eq, Show)

-- | The regenesis represents a reset of the protocol with a new genesis block.
--   This does not include the full new state, but only its hash.
--
--  The relationship between the new state and the state of the
--  terminal block of the old chain should be defined by the
--  chain update mechanism used.
--
--  There are two variants, one when migrating from 'P10' to 'P11' and
--  one from 'P11' to 'P11'.
data RegenesisP11
    = GDP11Regenesis {genesisRegenesis :: !BaseV1.RegenesisDataV1}
    | GDP11RegenesisFromP10
        { genesisRegenesis :: !BaseV1.RegenesisDataV1,
          genesisMigration :: !StateMigrationData
        }
    deriving (Eq, Show)

-- | Deserialize genesis data in the V13 format.
getGenesisDataV13 :: Get GenesisDataP11
getGenesisDataV13 =
    getWord8 >>= \case
        0 -> do
            genesisCore <- get
            genesisInitialState <- get
            return GDP11Initial{..}
        _ -> fail "Unrecognized P11 genesis data type."

getRegenesisData :: Get RegenesisP11
getRegenesisData =
    getWord8 >>= \case
        1 -> do
            genesisRegenesis <- get
            return GDP11Regenesis{..}
        2 -> do
            genesisRegenesis <- get
            genesisMigration <- get
            return GDP11RegenesisFromP10{..}
        _ -> fail "Unrecognized P11 regenesis data type."

-- | Serialize genesis data in the V13 format.
putGenesisDataV13 :: Putter GenesisDataP11
putGenesisDataV13 GDP11Initial{..} = do
    putWord8 0
    put genesisCore
    put genesisInitialState

-- | Deserialize genesis data with a version tag. The expected version tag is 12
--  and this must be distinct from version tags of other genesis data formats.
getVersionedGenesisData :: Get GenesisDataP11
getVersionedGenesisData =
    getVersion >>= \case
        13 -> getGenesisDataV13
        n -> fail $ "Unsupported genesis data version for P11 genesis: " ++ show n

-- | Serialize genesis data with a version tag.
--  This will use the V13 format.
putVersionedGenesisData :: Putter GenesisDataP11
putVersionedGenesisData gd = do
    putVersion 13
    putGenesisDataV13 gd

-- | Compute the block hash of the genesis block with the given genesis data.
--  Every block hash is derived from a message that begins with the block slot,
--  which is 0 for genesis blocks.
--
--  NB: For the regenesis variant the serialized state is not included in the
--  block hash, only the state hash is. This makes it possible to optimize the
--  format in the future since it does not have protocol defined meaning. In
--  contrast, for the initial P11 genesis the initial state is hashed as is.
genesisBlockHash :: GenesisDataP11 -> BlockHash
genesisBlockHash GDP11Initial{..} = BlockHash . Hash.hashLazy . runPutLazy $ do
    put genesisSlot
    put P11
    putWord8 0 -- initial variant
    put genesisCore
    put genesisInitialState

-- | Compute the block hash of the regenesis data as defined by the specified
--  protocol. This becomes the block hash of the genesis block of the new chain
--  after the protocol update.
regenesisBlockHash :: RegenesisP11 -> BlockHash
regenesisBlockHash GDP11Regenesis{genesisRegenesis = BaseV1.RegenesisDataV1{..}} = BlockHash . Hash.hashLazy . runPutLazy $ do
    put genesisSlot
    put P11
    putWord8 1 -- regenesis variant
    put genesisCore
    put genesisFirstGenesis
    put genesisPreviousGenesis
    put genesisTerminalBlock
    put genesisStateHash
regenesisBlockHash GDP11RegenesisFromP10{genesisRegenesis = BaseV1.RegenesisDataV1{..}, ..} = BlockHash . Hash.hashLazy . runPutLazy $ do
    put genesisSlot
    put P11
    putWord8 2 -- migration from P10 variant
    put genesisCore
    put genesisFirstGenesis
    put genesisPreviousGenesis
    put genesisTerminalBlock
    put genesisStateHash
    put genesisMigration

-- | The hash of the first genesis block in the chain.
firstGenesisBlockHash :: RegenesisP11 -> BlockHash
firstGenesisBlockHash GDP11Regenesis{genesisRegenesis = BaseV1.RegenesisDataV1{..}} = genesisFirstGenesis
firstGenesisBlockHash GDP11RegenesisFromP10{genesisRegenesis = BaseV1.RegenesisDataV1{..}} = genesisFirstGenesis

-- | Tag of the genesis data used for serialization.
genesisVariantTag :: GenesisDataP11 -> Word8
genesisVariantTag GDP11Initial{} = 0

-- | Tag of the regenesis variant used for serialization. This tag determines
--  whether the genesis data is, e.g., initial genesis, or regenesis and allows
--  us to deserialize one or the other from the data without knowing a priori what
--  the data is.
regenesisVariantTag :: RegenesisP11 -> Word8
regenesisVariantTag GDP11Regenesis{} = 1
regenesisVariantTag GDP11RegenesisFromP10{} = 2
