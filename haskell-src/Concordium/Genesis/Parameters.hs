...
-- | 'GenesisParametersV3' provides a convenient abstraction for
-- constructing 'GenesisData'. The following invariants are
-- required to hold:
--
-- * There must be at least one baker account in 'gpInitialAccounts'.
...
--   correspond to an account in 'gpInitialAccounts'.
--
-- This version is used for consensus version 1.
data GenesisParametersV3 pv = GenesisParametersV3
    { -- |Time at which genesis occurs.
      gp3GenesisTime :: Timestamp,
      -- |Initial nonce for seeding the leadership election.
      gp3LeadershipElectionNonce :: LeadershipElectionNonce,
      -- |Duration of an epoch.
      gp3EpochDuration :: Duration,
      -- |Cryptographic parameters.
      gp3CryptographicParameters :: CryptographicParameters,
      -- |The identity providers present at genesis.
      gp3IdentityProviders :: IdentityProviders,
      -- |The anonymity revokers present at genesis.
      gp3AnonymityRevokers :: AnonymityRevokers,
      -- |Initial accounts. Since an account can be a baker, it is important that the
      -- order of the accounts matches the assigned baker ids.
      gp3InitialAccounts :: [GenesisAccount],
      -- |The collection of update keys for performing updates
      gp3UpdateKeys :: UpdateKeysCollection (AuthorizationsVersionFor (ChainParametersVersionFor pv)),
      -- |The initial (updatable) chain parameters
      gp3ChainParameters :: GenesisChainParameters pv
    }

instance forall pv. IsProtocolVersion pv => FromJSON (GenesisParametersV3 pv) where
    parseJSON = withObject "GenesisParametersV3" $ \v -> do
        gp3GenesisTime <- v .: "genesisTime"
        gp3LeadershipElectionNonce <- v .: "leadershipElectionNonce"
        gp3EpochDuration <- v .: "epochDuration"
        when (gp3EpochDuration == 0) $ fail "Epoch duration should be non-zero"
        gp3CryptographicParameters <- v .: "cryptographicParameters"
        gp3IdentityProviders <- v .:? "identityProviders" .!= emptyIdentityProviders
        gp3AnonymityRevokers <- v .:? "anonymityRevokers" .!= emptyAnonymityRevokers
        gp3InitialAccounts <- v .:? "initialAccounts" .!= []
        let hasBaker GenesisAccount{gaBaker = Nothing} = False
            hasBaker _ = True
        unless (any hasBaker gp3InitialAccounts) $ fail "Must have at least one baker at genesis"
        let validateBaker (bid, GenesisAccount{gaBaker = Just bkr}) =
                unless (gbBakerId bkr == bid) $ fail $ "Expected baker id " ++ show bid ++ " but was " ++ show (gbBakerId bkr)
            validateBaker _ = return ()
        mapM_ validateBaker (zip [0 ..] gp3InitialAccounts)
        gp3UpdateKeys <-
            withIsAuthorizationsVersionForPV (protocolVersion @pv) $
                v .: "updateKeys"
        gp3ChainParameters <- v .: "chainParameters"
        let facct = gcpFoundationAccount gp3ChainParameters
        unless (any ((facct ==) . gaAddress) gp3InitialAccounts) $
            fail $
                "Foundation account (" ++ show facct ++ ") is not in initialAccounts"
        return GenesisParametersV3{..}
