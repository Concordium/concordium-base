...
instance HashableTo TransactionOutcomesHashV0 TransactionOutcomes where
    getHash transactionoutcomes = TransactionOutcomesHashV0 $ H.hash $ S.runPut $ putTransactionOutcomes . transformOutcomes $ transactionoutcomes
      where -- Transform the transaction outcomes to work around a bug in the initial implementation of P4 where an incorrect event was logged
            -- for V1 smart contract to an account transfer.
            transformOutcomes TransactionOutcomes{..} = TransactionOutcomes{
            ...
            -- We need to transform the events generated by successful invocations of V1 smart contract updates only.
            transformResult trType ts@TxSuccess {..}
                | trType /= TSTAccountTransaction (Just TTUpdate) = ts
                -- Go through the list in reverse order and transform all relevant "Transferred" events.
                | otherwise = TxSuccess (snd $ foldl' transformEvent ([], []) . reverse $ vrEvents)

            -- This function is the real worker of the transformation. It takes as a parameter a pair of
            -- - a stack of V1 contract addresses together with the amount that the contract was invoked with
            -- - a list of processed events
            --
            -- This function relies on the following property of generated
            -- events. Every successful "Transferred" event from a V1 contract
            -- address A is followed by an "Updated" event which contains the
            -- "transferAmount" that used to mistakenly be in the "Transferred"
            -- event. Now, because of nested contract invocations we must have a
            -- stack. We push the address and amount onto the stack when we see
            -- an "Updated" event for a V1 smart contract. We duplicate the top element of the stack
            -- at a Resumed event, which will always follow a V1 Transferred event, and we pop from the stack
            -- on an Interrupted event, which will always precede the V1 Transferred event.
            ...
            -- The check from == from' ensures that we only transform Transferred events from V1 contracts.
            transformEvent (stack@((from, transferAmount):_), rest) Transferred{etFrom=fromAddress@(AddressContract from'),..}
                | from == from' = (stack, Transferred{etAmount = transferAmount,etFrom = fromAddress, ..}:rest)
            transformEvent (s:stack, rest) ev@Resumed{} = (s:s:stack, ev:rest)
            transformEvent (s:stack, rest) ev@Interrupted{} = (stack, ev:rest)
