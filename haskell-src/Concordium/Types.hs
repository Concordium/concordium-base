...
instance S.Serialize Release where
  put Release{..} = do
    S.put timestamp
    S.put amount
  get = do
...
getHashOfReleases :: [Release] -> Hash.Hash
getHashOfReleases [] = error "Unreachable"
getHashOfReleases (x:[]) = Hash.hash $ S.encode x
getHashOfReleases (x:xs) =
  let xSerialized = S.encode x
      hashOfNext = getHashOfReleases xs
  in
    Hash.hash (xSerialized <> Hash.hashToByteString hashOfNext)
...
instance S.Serialize AccountReleaseSchedule where
  get = do
    vecLength <- getLength
    _values <- Vector.replicateM vecLength (getMaybe (do
                                                         l <- getLength
                                                         item <- sequence $ replicate l S.get
                                                         txh <- S.get
                                                         return (item, txh)))
...
newtype AccountReleaseScheduleHash = AccountReleaseScheduleHash Hash.Hash
 deriving (S.Serialize, Eq, Ord, Show)

emptyAccountReleaseScheduleHash :: AccountReleaseScheduleHash
emptyAccountReleaseScheduleHash = AccountReleaseScheduleHash (Hash.hash "EmptyAccountReleaseSchedule")
...
instance HashableTo AccountReleaseScheduleHash AccountReleaseSchedule where
  getHash AccountReleaseSchedule{..} =
    ...
    else AccountReleaseScheduleHash $ Hash.hash $ Vector.foldl' (\prevB -> \case
                                    Nothing -> prevB
                                    Just (r, _) -> prevB <> Hash.hashToByteString (getHashOfReleases r)) BS.empty _values
