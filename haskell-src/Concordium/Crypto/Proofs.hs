...
checkDlog25519Proof :: BS.ByteString -- ^The challenge prefix to use
...
checkDlog25519ProofVRF :: BS.ByteString -- ^The challenge prefix to use
                       -> VRF.PublicKey -- ^The VRF public key.
                       -> Dlog25519Proof -- ^Purported proof.
                       -> Bool
...
proveDlog25519 challenge publicKey secretKey = 
  BS.unsafeUseAsCStringLen challenge $ \(c_ptr, c_len) ->
    BS.unsafeUseAsCStringLen publicKey $ \(pk_ptr, pk_len) ->
     if pk_len /= 32 then return Nothing
     else BS.unsafeUseAsCStringLen secretKey $ \(sk_ptr, sk_len) ->
       if sk_len /= 32 then return Nothing
       else do
