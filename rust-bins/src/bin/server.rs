...
// use id::{account_holder::*, identity_provider::*, secret_sharing::Threshold,
// types::*}; use pairing::bls12_381::Bls12;
// use rand::*;
...
// fn respond_global_params(_request: &rouille::Request, s: &ServerState) ->
// rouille::Response {     let response =
// to_string_pretty(&s.global_params).unwrap();     rouille::Response::
// text(response) }

// fn respond_ips(_request: &rouille::Request, s: &ServerState) ->
// rouille::Response {     let response: Vec<IpInfo<_, _>> = s
//         .ip_infos
//         .iter()
...
// fn respond_id_object(request: &rouille::Request, s: &ServerState) ->
// rouille::Response {     let v: Value =
// try_or_400!(rouille::input::json_input(request));     let (ip_info,
// threshold, ar_identities, attributes) = {         if let Some((ip_id,
// threshold, ar_list, att)) = parse_id_object_input_json(&v) {
// match s.ip_infos.get(&ip_id) {                 Some(ip_info) => (ip_info,
// threshold, ar_list, att),                 None => return
// respond_log!(request, "Could not find identity provider."),             }
//         } else {
...
//     let context = match make_context_from_ip_info(ip_info.clone(),
// ChoiceArParameters {         ar_identities,
//         threshold,
//     }) {
//         Some(x) => x,
...
//         v
//     } else {
//         return respond_log!(request, "Could not generate pre-identity
// object.");     };
...
//             rouille::Response::json(&response)
//         }
//         Err(e) => respond_log!(request, format!("Could not generate
// credential: {:?}", e)),     }
// }
...
// fn parse_generate_credential_input_json(v: &Value) ->
// Option<GenerateCredentialData> {     let ip_id =
// from_json(v.get("ipIdentity")?.clone()).ok()?;     let id_object:
// IdentityObject<Bls12, ExampleCurve, ExampleAttribute> = v         .get("
// identityObject")         .and_then(|x| from_json(x.clone()).ok())?;
...
// fn respond_generate_credential(request: &rouille::Request, s: &ServerState)
// -> rouille::Response {     let v: Value =
...
//                 Some(ref ip_info) => {
//                     let policy: Policy<ExampleCurve, ExampleAttribute> =
// Policy {                         valid_to:   id_object.alist.valid_to,
...
//                 Err(e) => {
//                     return respond_log!(request, format!("Could not parse
// account data {}.", e))                 }
//             }
//         } else {
//             let mut keys = BTreeMap::new();
//             let mut csprng = thread_rng();
//             keys.insert(KeyIndex(0), ed25519::Keypair::generate(&mut
// csprng));             keys.insert(KeyIndex(1),
// ed25519::Keypair::generate(&mut csprng));
...
//     let cdi = match cdi {
//         Ok(cdi) => cdi,
//         Err(e) => return respond_log!(request, format!("Could not generate
// credential {}", e)),     };
...
// pub fn main() {
//     let app = App::new("Server exposing creation of identity objects and
// credentials")         .version("0.31830988618")
//         .author("Concordium")
...
// .value_name("FILE")                 .help("File with public and private
// information on IPs and ARs."),         )
//         .arg(
...
//         } else {
//             eprintln!(
//                 "Could not read global cryptographic parameters from {}.
// Aborting.",                 gc_file
//             );
//             return;
//         }
...
//                 (GET) (/globalparams) => { respond_global_params(request,
// &ss) },                 // get public identity provider info
//                 (GET) (/ips) => { respond_ips(request, &ss) },
//                 // Given an attribute list generate a freshly signed identity
// object, together                 // with all the private data the account
// holder would normally generate themselves.                 (POST)
// (/identity_object) => { respond_id_object(request, &ss) },                 //
// Generate a credential to be deployed on the chain.                 (POST)
// (/generate_credential) => { respond_generate_credential(request, &ss) },
