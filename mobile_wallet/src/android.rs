...
use crate::{
    ...
    get_verifiable_credential_keys, parameter_to_json, prove_id_credential, prove_id_statement,
...
#[no_mangle]
/// The JNI wrapper for the `prove_id_credential` method.
/// The `input` parameter must be a properly initalized `java.lang.String` that
/// is non-null. The input must be valid JSON according to specified format
pub extern "system" fn Java_com_concordium_mobile_1wallet_1lib_WalletKt_prove_1id_1credential(
    env: JNIEnv,
    _: JClass,
    input: JString,
) -> jobject {
    ...
    let mut success: u8 = 127;
    let cstr_res = unsafe {
        let unsafe_res_ptr = prove_id_credential(input_str.as_ptr(), &mut success);
        if unsafe_res_ptr.is_null() {
            return wrap_return_tuple(&env, 127, "Pointer returned from crypto library was NULL");
        }
        CString::from_raw(unsafe_res_ptr)
