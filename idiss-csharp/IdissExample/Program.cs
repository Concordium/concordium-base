using System;
using System.IO;
using System.Text.Json;
using IdissLib;
using System.Collections.Generic;

namespace IdissExample
{
    class Program
    {
        static void Main(string[] args)
        {
            StreamReader r = new StreamReader("../data/global.json");
            string globalStr = r.ReadToEnd();
            var global = JsonSerializer.Deserialize<Versioned<GlobalContext>>(globalStr);
            StreamReader r2 = new StreamReader("../data/valid_request.json");
            string requestStr = r2.ReadToEnd();
            var request = JsonSerializer.Deserialize<IdObjectRequest>(requestStr);
            StreamReader r3 = new StreamReader("../data/anonymity_revokers.json");
            string arsInfosStr = r3.ReadToEnd();
            var arsInfos = JsonSerializer.Deserialize<Versioned<Dictionary<string, ArInfo>>>(arsInfosStr);
            StreamReader r4 = new StreamReader("../data/identity_provider.pub.json");
            string ipInfoStr = r4.ReadToEnd();
            var ipInfo = JsonSerializer.Deserialize<Versioned<IpInfo>>(ipInfoStr);


            var options = new JsonSerializerOptions();
            options.Converters.Add(new DictionaryConverter());
            options.Converters.Add(new YearMonthConverter());

            StreamReader r5 = new StreamReader("../data/alist.json");
            string alistStr = r5.ReadToEnd();
            var alist = JsonSerializer.Deserialize<AttributeList>(alistStr, options);

            StreamReader r6 = new StreamReader("../data/valid_request_v1.json");
            string requestV1Str = r6.ReadToEnd();
            var requestV1 = JsonSerializer.Deserialize<IdObjectRequestV1>(requestV1Str);

            StreamReader r7 = new StreamReader("../data/id_recovery_request.json");
            string recoveryRequestStr = r7.ReadToEnd();
            var recoveryRequest = JsonSerializer.Deserialize<IdRecoveryWrapper>(recoveryRequestStr);

            var ipKeys = new IpPrivateKeys
            {
                ipPrivateKey = "",
                ipCdiPrivateKey = "c7a0e9394dbed945ec1a8ea160fce0901ce3867f3c0d969318b9c0f777ccb2f4"
            };

            UInt64 expiry = 1234567;
            try
            {
                var result = Idiss.ValidateRequest(global, ipInfo, arsInfos, request);
                Console.WriteLine("Version 0 validation was successful. Address: ");
                Console.WriteLine(result);
                var idCreation = Idiss.CreateIdentityObject(ipInfo, alist, request, expiry, ipKeys);
                var idCreationJson = JsonSerializer.Serialize(idCreation, options);
                Console.WriteLine("Id creation JSON output:");
                Console.WriteLine(idCreationJson);
            }
            catch (RequestValidationException e)
            {
                Console.WriteLine("Validation failed:");
                Console.WriteLine(e.Message);
            }
            catch (IdentityCreationException e)
            {
                Console.WriteLine("Identity creation failed:");
                Console.WriteLine(e.Message);
            }

            try
            {
                Idiss.ValidateRequestV1(global, ipInfo, arsInfos, requestV1);
                Console.WriteLine("Version 1 validation was successful.");
                var idCreation = Idiss.CreateIdentityObjectV1(ipInfo, alist, requestV1, ipKeys);
                var idCreationJson = JsonSerializer.Serialize(idCreation, options);
                Console.WriteLine("Version 1 Id creation JSON output:");
                Console.WriteLine(idCreationJson);
            }
            catch (RequestValidationException e)
            {
                Console.WriteLine("Version 1 validation failed:");
                Console.WriteLine(e.Message);
            }
            catch (IdentityCreationException e)
            {
                Console.WriteLine("Version 1 identity creation failed:");
                Console.WriteLine(e.Message);
            }

            try
            {
                long timestamp = Convert.ToInt64(recoveryRequest.idRecoveryRequest.value.timestamp);
                var idpTime = DateTimeOffset.FromUnixTimeSeconds(timestamp);
                Idiss.ValidateRecoveryRequest(global, ipInfo, recoveryRequest, idpTime);
                Console.WriteLine("Recovery validation was successful.");
            }
            catch (RequestValidationException e)
            {
                Console.WriteLine("Recovery validation failed:");
                Console.WriteLine(e.Message);
            }

            Console.ReadLine();
        }
    }
}
